...
object.sort(function(x,y) {return x.a - y.a});
console.log(object);
/*
    forEach(콜백 함수) : for 문 대신 사용
    - 배열을 순회하면서 각 요소와 인덱스를 사용할 수 있도록 함
    - 원본 배열을 변경 X
    - break, continue 등의 작업을 수행 할 수 없음
...
/*
    map(콜백 함수) : 콜백 함수의 반환값들로 이루어진 새로운 배열을 생성해서 변환
    - 원본 배열 변경 X
...
// var mapArray = array.map(function('$ , _', index){
//     return item + index;
// }); -> 인자를 하나만 보내줄때 $, _로 표기는 해주어야 한다.
...
/*
    filter(콜백 함수) : 콜백 함수의 반환값이 true인 배열의 요소의 값만 추출한 새로운 배열을 생성합니다.
    - 배열의 특정 케이스만 필터링 조건으로 추출해서 새로운 배열을 만들고 싶을 때 사용
    - 원본 배열 변경X
*/
var evenArray = array.filter(function(item,index){return item%2 === 0})
console.log(evenArray)
//filter을 일반 for 문으로 구성***
//1.빈 배열 생성
var newArray =[];
//2.for문 작성
for(var index in array){
